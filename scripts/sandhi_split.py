...
def split_sandhi(in_word, in_encoding, out_encoding):
    ...
    word_path = main_path + "/" + in_word + "_sandhi"
    try:
        os.mkdir(word_path)
    except:
        pass
    ...
    outfile = open(word_path + "/san_most_probable_output.txt", "w")
    ...
    tmpout = open(word_path + "/san_tmpout.txt", "w")
    tmpout.write(ps.stdout.decode("utf-8"))
    tmpout.close()

    out = subprocess.run(
        ["cut", "-f1", word_path + "/san_tmpout.txt"],
        cwd=word_path,
        stdout=subprocess.PIPE,
    )

    finout = open(word_path + "/san_finout.txt", "w")
    finout.write(out.stdout.decode("utf-8"))
    finout.close()
    output = ""

    with open(word_path + "/san_finout.txt", "r") as finalout:
